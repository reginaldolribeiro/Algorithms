...
//        System.out.println(s2.codePointAt(s2.length()-1));
    ...
    public static boolean isAnagram(String s, String t){
        ...
        for (int i = 0; i < s.length(); i++) {
            int unicodeIndexFirstString = s.codePointAt(i);
            map.put(unicodeIndexFirstString, map.getOrDefault(unicodeIndexFirstString, 0)+1);
            int unicodeIndexSecondString = t.codePointAt(i);
            map.put(unicodeIndexSecondString, map.getOrDefault(unicodeIndexSecondString,0)-1);
        }
     ...
     * Using an array of Character.MAX_CODE_POINT we have 1,114,111 positions to iterate.
     *
     * @param s
     * @param t
     * @return
    ...
    public static boolean isAnagram1(String s, String t){
        ...
        int[] charFrequencies = new int[Character.MAX_CODE_POINT]; // Is not efficient because Character.MAX_CODE_POINT is 1,114,111 positions.
        for (int i = 0; i < s.length(); i++) {
            int indexFirstString = s.codePointAt(i); // get the index of the character inside the Unicode array
            charFrequencies[indexFirstString]++;
            int indexSecondString = t.codePointAt(i);
