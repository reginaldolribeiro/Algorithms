...
public class LoadBalancerThreadSafe {
    private List<LoadBalancerInstance> instances;
    private int nextInstance = 0;

    public LoadBalancerThreadSafe(List<LoadBalancerInstance> instances){
        this.instances = Collections.synchronizedList(new ArrayList<>(instances)); // Ensure mutability
    }

    public synchronized List<LoadBalancerInstance> getInstances() {
        return instances;
    }

    public synchronized void registerInstance(LoadBalancerInstance newInstance){
        this.instances.add(newInstance);
    }



    public synchronized void deregisterInstance(LoadBalancerInstance newInstance){
        this.instances.remove(newInstance);
    }

    public synchronized void registerInstances(List<LoadBalancerInstance> instances){
        this.instances.addAll(instances);
    }

    public synchronized void deregisterInstances(List<LoadBalancerInstance> instances){
